...
- **OpenClaw heartbeat 关闭**: 每 30 分钟的 "Read HEARTBEAT.md" 系统消息不再出现
- **关闭 4 个非消费级功能**: Bonjour/mDNS 网络广播、浏览器控制(18791端口)、OpenClaw 自动更新检查、Canvas 文件服务器

### Changed

- **模型描述优化**: MiniMax(推荐/中文编程)、DeepSeek(老牌/性价比)、Kimi(超强上下文)、通义千问(免费额度)、智谱(清华系/编程)、Gemini(多模态)
- **模型选择器浮层化**: 下拉框改为悬浮覆盖（不推挤页面），高度增加 50%
- **频道配置引导**: 每个频道卡片顶部增加"第一步：前往平台创建应用"链接
- **代理设置提示**: 增加格式示例和"留空使用系统代理"说明
- **Gateway 启动提示**: "检测中..."改为"正在启动 AI 引擎，通常需要 10-20 秒"
- **Skills 提示**: 落地页显示"已预装 66 个 AI 技能"
- **海外模型标签**: Anthropic/GPT/Gemini 卡片标记"需海外网络"，验证失败提示检查代理
- **一键更新失败**: 手动更新方法自动展开
